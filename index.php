...
// xPlugin Name: Media Library Categories
// xPlugin URI: http://wordpress.org/plugins/wp-media-library-categories/
// xDescription: Adds the ability to use categories in the media library.
// xVersion: 1.5.1
// xAuthor: Jeffrey-WP
// xAuthor URI: http://codecanyon.net/user/jeffrey-wp/?ref=jeffrey-wp
...
if ( is_admin() ) {
		...
		if ( 'upload.php' == $pagenow ) {
			...
			if ( $taxonomy != 'category' ) {
				$dropdown_options = array(
					'taxonomy'        => $taxonomy,
					'name'            => $taxonomy,
					'show_option_all' => __( 'View all categories' ,PO),
					'hide_empty'      => false,
					'hierarchical'    => true,
					'orderby'         => 'name',
					'show_count'      => true,
	...
	function wpmediacategory_custom_bulk_admin_footer() {
		...
		if ( $terms && ! is_wp_error( $terms ) ) :
			...
			echo 'jQuery(\'<optgroup id="wpmediacategory_optgroup1" label="' .  html_entity_decode( __( 'Categories',PO ), ENT_QUOTES, 'UTF-8' ) . '">\').appendTo("select[name=\'action\']");';
			echo 'jQuery(\'<optgroup id="wpmediacategory_optgroup2" label="' .  html_entity_decode( __( 'Categories',PO ), ENT_QUOTES, 'UTF-8' ) . '">\').appendTo("select[name=\'action2\']");';

			// add categories
			foreach ( $terms as $term ) {
				$sTxtAdd = esc_js( __( 'Add' ,PO) . ': ' . $term->name );
			...
			// remove categories
			foreach ( $terms as $term ) {
				$sTxtRemove = esc_js( __( 'Remove',PO ) . ': ' . $term->name );
			...
			echo "jQuery('<option>').val('wpmediacategory_remove_0').text('" . esc_js(  __( 'Delete all',PO ) ) . "').appendTo('#wpmediacategory_optgroup1');";
			echo "jQuery('<option>').val('wpmediacategory_remove_0').text('" . esc_js(  __( 'Delete all',PO ) ) . "').appendTo('#wpmediacategory_optgroup2');";
			echo "});";
			echo "</script>";
	...
	function wpmediacategory_custom_bulk_admin_notices() {
		...
		if ( $pagenow == 'upload.php' && $post_type == 'attachment' && isset( $_GET['editCategory'] ) ) {
			echo '<div class="updated"><p>' . __( 'Settings saved.' ,PO) . '</p></div>';
		}
	}
	...
	function wpmediacategory_add_plugin_action_links( $links ) {
		...
		return array_merge(
			array(
				'settings' => '<a href="' . get_bloginfo( 'wpurl' ) . '/wp-admin/edit-tags.php?taxonomy=' . $taxonomy . '&amp;post_type=attachment">' . __( 'Categories' ,PO) . '</a>',
				'premium' => '<a href="http://codecanyon.net/item/media-library-categories-premium/6691290?ref=jeffrey-wp">' . __( 'Get Premium Version',PO ) . '</a>'
			),
			$links
		);
	}
	...
	function wpmediacategory_enqueue_media_action() {
		...
		if ( wp_script_is( 'media-editor' ) && 'upload.php' == $pagenow ) {
			...
			echo '<script type="text/javascript">';
			echo '/* <![CDATA[ */';
			echo 'var wpmediacategory_taxonomies = {"' . $taxonomy . '":{"list_title":"' . html_entity_decode( __( 'View all categories',PO ), ENT_QUOTES, 'UTF-8' ) . '","term_list":[' . substr( $attachment_terms, 2 ) . ']}};';
			echo '/* ]]> */';
			echo '</script>';
